...
### Touch / Mobile
- **Native `date`/`time`-Felder auf iOS:** ignorieren `width:100%`, nehmen ihre Eigenbreite und ragen über den Rand ("Feld klebt am Rand / zu groß"). Fix: `-webkit-appearance: none; appearance: none; min-width: 0;` auf `input[type="date"|"time"]` → füllen dann die Containerbreite. Wert links via `::-webkit-date-and-time-value { text-align:left }`, Picker-Icon mit `margin-left:4px; flex-shrink:0`. (Hinweis: in Desktop-Chrome unsichtbar, weil Chrome `width:100%` ohnehin respektiert — nur am echten iPhone testen.)
